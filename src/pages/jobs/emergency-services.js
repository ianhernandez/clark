  ...
  return (
		<>
			<PageBasic>
				...
				<PageHeader title="Emergency Services Jobs" breadcrumbs={breadcrumbs} />
					...
					<Container>
						<div className="grid md:grid-cols-12 md:gap-x-5">
							<div className="flex flex-col md:col-span-12 lg:col-span-6 gap-y-5">
								{company.map((item) => (
								<JobsListItem key={item.link} title={item.title} location={item.location} description={item.description} link={item.link} list={company} />
								))}
							</div>
							<div className="p-6 bg-white rounded-lg lg:col-span-6">
									<h1 className="text-2xl font-semibold prose">{company[0].title}</h1>
